...
module.exports = (io)=>{
    var players = {};
    var synchronizedBodies = true;
    var bodies = [];
    var lastSocketSentBodies = {broadcast:{emit:()=>{}}};
    let intervalId = setInterval(()=>{
          io.emit('update-players',players);
          if(synchronizedBodies && Object.keys(players).length > 1) lastSocketSentBodies.broadcast.emit('update-bodies',bodies);
        },100);
    io.on('connection', function(socket){
        ...
        socket.on('update-bodies',function(data){
          //console.log(data);
          synchronizedBodies = true;
          lastSocketSentBodies = socket;
          bodies = data;
        });
